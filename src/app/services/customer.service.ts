...
export class CustomerService {
  ...
  getRegions(): Observable<Region[]> {
    return this.http.get<Region[]>(this.urlEndPoint +'/regions', {headers: this.addAuthorizationHeader()}).pipe(
      catchError(e => {
        this.isNoAuthorized(e);
        //console.log(this.addAuthorizationHeader());
        return throwError(e);
      })
    );
  }
  ...
  create(customer: Customers) : Observable<any> {
    return this.http.post<Customers>(this.urlEndPoint, customer, {headers: this.addAuthorizationHeader()}).pipe(
      catchError(e => {
  ...
  getCustomers(id: number): Observable<Customers[]> {
    return this.http.get<Customers[]>(`${this.urlEndPoint}/${id}`, {headers: this.addAuthorizationHeader()}).pipe(
      catchError(e => {
        if (this.isNoAuthorized(e)) {
            return throwError(e);
        }
  ...
  delete(id: number): Observable<Customers> {
    return this.http.delete<Customers>(`${this.urlEndPoint}/${id}`, {headers: this.addAuthorizationHeader()}).pipe(
      catchError(e => {
        if (this.isNoAuthorized(e)) {
            return throwError(e);
        }
